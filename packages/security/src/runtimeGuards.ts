...
export class RuntimeSecurityGuards {
   ...
   */
  checkPermission(module: string, operation: string, details: Record<string, unknown> = {}): boolean {
    try {
   ...
   */
  checkRateLimit(module: string, operation: string): boolean {
   ...
   */
  detectPrivilegeEscalation(module: string, requestedLevel: PermissionLevel, currentLevel: PermissionLevel): boolean {
    if (!this.config.privilegeEscalationDetection) return true;
   ...
   */
  checkFileAccess(module: string, operation: string, filePath: string): boolean {
    if (!this.config.fileSystemAccessControl) return true;
    ...
    const isSensitivePath = sensitivePaths.some(path => 
      filePath.includes(path) || filePath.endsWith(path)
    );
   ...
   */
  checkNetworkAccess(module: string, url: string, method: string = 'GET'): boolean {
    if (!this.config.networkAccessControl) return true;
   ...
   */
  private recordViolation(violation: Omit<SecurityViolation, 'timestamp' | 'stackTrace'>): void {
   ...
   */
  getViolations(level?: 'info' | 'warning' | 'critical' | 'fatal'): SecurityViolation[] {
    if (level) {
   ...
   */
  getModulePermissions(): Map<string, ModulePermission> {
   ...
   */
  setModulePermission(permission: ModulePermission): void {
